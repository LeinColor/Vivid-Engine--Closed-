...
void Renderer::Render()
{
	...
	for (int i = 0; i < Scene::objects.size() - 1; i++) {
		...
		if (Scene::objects[i]->GetDebugFlag() == false) {
			...
			graphics->GetContext()->IASetInputLayout(Manager::inputLayouts[1]);
			graphics->GetContext()->VSSetShader(Manager::vertexShaders[1], nullptr, 0);
			graphics->GetContext()->PSSetShader(Manager::pixelShaders[1], nullptr, 0);
		...
		else if (Scene::objects[i]->GetDebugFlag() == true)
		{
			...
			// Bind pipeline before drawing
			graphics->GetContext()->IASetInputLayout(Manager::inputLayouts[0]);
			graphics->GetContext()->VSSetShader(Manager::vertexShaders[0], nullptr, 0);
			graphics->GetContext()->PSSetShader(Manager::pixelShaders[0], nullptr, 0);
			graphics->GetContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		}
...
void Renderer::DrawLine(XMFLOAT3 startPoint, XMFLOAT3 endPoint, XMFLOAT4 color)
{
	...
	graphics->GetContext()->Map(*Scene::meshes[MESH_LINE]->GetVertexBuffer(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	verticesPtr = (vertex_PCN_t*)mappedResource.pData;
	memcpy(verticesPtr, (void*)vertices, (sizeof(vertex_PCN_t) * 2));
	graphics->GetContext()->Unmap(*Scene::meshes[MESH_LINE]->GetVertexBuffer(), 0);
	...
	graphics->GetContext()->IASetVertexBuffers(0, 1, Scene::meshes[MESH_LINE]->GetVertexBuffer(), &stride, &offset);
	graphics->GetContext()->IASetIndexBuffer(*Scene::meshes[MESH_LINE]->GetIndexBuffer(), DXGI_FORMAT_R32_UINT, 0);
