...
namespace ComponentSystem
{
	...
	void Update(Camera& camera)
	{
		...
		XMStoreFloat3(&camera.eye, vEye);
		XMStoreFloat3(&camera.focus, vFocus);
		XMStoreFloat3(&camera.up, vUp);

		XMMATRIX viewMatrix = XMMatrixLookAtLH(vEye, vFocus, vUp);
		XMMATRIX projectionMatrix = XMMatrixPerspectiveFovLH(camera.fieldOfView, 1024.0f / 768.0f, camera.nearZ, camera.farZ);
		XMMATRIX orthoMatrix = XMMatrixOrthographicLH(1024.0f, 768.0f, camera.nearZ, camera.farZ);
		XMStoreFloat4x4(&camera.view, viewMatrix);
		XMStoreFloat4x4(&camera.projection, projectionMatrix);
